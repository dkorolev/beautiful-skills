...
- The commits that get pushed must be authored (and committed) as this human's GitHub identity:

  | GitHub login | Required author email | Author name |
  |---|---|---|
  | `dimacurrentai` | `dima@xmemory.ai` | Use `git config user.name`, or `gh api user -q .name` if unset |
  | any other login | Must match a verified email on the GitHub account | Use `git config user.name`, or `gh api user -q .name` if unset |

  For non-`dimacurrentai` logins, confirm the chosen email appears in `gh api user/emails --jq '.[].email'` (or accept the user's primary GitHub email from `gh api user -q .email` when it is verified). If you cannot determine a valid pair, stop and ask once.
